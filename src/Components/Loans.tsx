...
const Loans = ({role}:{role:string}) => {
    ...
    const [filter, setFilter] = useState({ maxPrice: 10000, lastDate: "" });
    ...
    useEffect(() => {
        const fetchLoans = async () => {
            try {
                let resp;
                if (finalRole === "user") {
                    resp = await axios.get(api_link + "/api/loans/user");
                } else if (finalRole === "admin" || finalRole === "verifier") {
                    resp = await axios.get(api_link + `/api/loans?role=${finalRole}`);
                } else {
                    setLoading(false);
                    return;
                }
